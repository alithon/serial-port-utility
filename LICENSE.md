# License

Copyright © 2026 Alithon Studio. All rights reserved.

Serial Port Utility is **commercial, proprietary software**. It is licensed, not sold. This
repository is a distribution channel for the released packages; it is not open source, and no
part of it grants rights to the application's source code.

The binding terms are the
[Terms of Service](https://alithon.com/terms) published at alithon.com. The summary below is
for convenience only — where the two differ, the Terms of Service govern.

## What you may do

- Use Free basic debugging without a paid license. Direct downloads include **30 days of Pro from first launch**; after the trial, Free remains available and saved project files and logs are kept.
- Use Pro under a license purchased from [alithon.com](https://alithon.com),
  within the edition, term, quantity and activation scope shown at checkout. A personal
  license covers the named purchaser and their permitted devices; an enterprise license
  covers the purchasing organization and the purchased quantity.
- Pass an unmodified installer on to a colleague who may use Free or obtain their own Pro license. Cloud traffic allowances and Mac App Store subscription offers are separate from the direct-download trial.

## What you may not do

- Resell, rent, sublicense or bundle the software into another product.
- Bypass, patch or emulate the licensing and activation mechanism, or share license keys.
- Remove or alter copyright, trademark or license notices.
- Reverse engineer, decompile or disassemble the software, except to the extent applicable
  law expressly permits it despite this restriction.
- Republish modified packages, or distribute builds that present themselves as official.
- Continue to use Pro features on a device after the license used to activate them has been refunded. Alithon may restrict a device that has been used to breach these terms; the software then displays a notice and exits on that device (Terms of Service, section 8).

## Documentation and images

The text and screenshots in this repository are © 2026 Alithon Studio. You may quote and link
to them for reviews, tutorials and comparisons with attribution; wholesale republication is
not permitted.

## No warranty

The software is provided on an "as available" basis. To the maximum extent permitted by law,
Alithon Studio disclaims implied warranties and is not liable for indirect, incidental,
special or consequential losses. Nothing here limits liability or consumer rights that cannot
legally be limited.

## Third-party components

Serial Port Utility incorporates open-source components, listed with their licenses in
[THIRD-PARTY-NOTICES.md](THIRD-PARTY-NOTICES.md).

## Questions

License, order and volume-purchase questions: <bill@alithon.com>

---

# 授权许可

Copyright © 2026 Alithon Studio. 保留所有权利。

友善串口调试助手（Serial Port Utility）是**商业专有软件**，为授权使用而非出售。本仓库是已发布
安装包的分发渠道，并非开源项目，也不授予任何对应用程序源代码的权利。

具有约束力的条款是 alithon.com 上发布的[服务条款](https://alithon.com/terms)。以下摘要仅供
参考；如有出入，以服务条款为准。

**可以**：无需付费授权使用 Free 基础调试；直接下载版从首次启动起提供 **30 天 Pro 试用**，结束后仍可使用 Free，已保存的工程文件和日志会保留。凭在 [alithon.com](https://alithon.com) 购买的授权，在结算页所示的版本、期限、数量与激活范围内使用 Pro；把未经修改的安装包转给可使用 Free 或自行购买 Pro 授权的同事。云流量额度和 Mac App Store 订阅优惠不等同于直接下载版试用。

**不可以**：转售、出租、再授权或将其打包进其它产品；绕过、修改或模拟授权与激活机制，或共享
授权密钥；删除或篡改版权、商标与许可声明；反向工程、反编译或反汇编（适用法律明确允许的范围
除外）；重新发布经过修改的安装包，或分发冒充官方的构建。

本仓库中的文字与截图版权归 Alithon Studio 所有。评测、教程与横向对比可在注明来源的前提下引用
并链接；不得整体转载。

软件按"现状可用"提供。在法律允许的最大范围内，Alithon Studio 不承担默示担保责任，也不对间接、
偶发、特殊或后果性损失负责。本节不限制依法不可限制的责任与消费者权利。

软件使用的开源组件及其许可证见 [THIRD-PARTY-NOTICES.md](THIRD-PARTY-NOTICES.md)。

授权、订单与批量采购问题请联系 <bill@alithon.com>。
