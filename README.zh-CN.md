...
这三条路本软件都提供，区别只在两处：**连接由哪一端发起**，以及**串口参数会不会跟着数据一起过去**。
...
| | RFC 2217 | 透明 TCP 转发 | 云控制台 |
| --- | --- | --- | --- |
| 网络前提 | 客户端能直达现场 IP 和端口 | 客户端能直达现场 IP 和端口 | 现场能上网即可，无需入站端口 |
| 远程修改串口参数 | 可以 | 不可以 | 可以 |
...
- **不能** —— 现场在 NAT 后面、没有公网地址也没有 VPN：只能走云控制台。为此做端口映射，
  等于把一台调试机暴露到公网，代价远大于收益。
...
三者并不互斥，常见做法是日常在局域网内用 RFC 2217，出差时用云控制台，
